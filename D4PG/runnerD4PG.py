...
def test_net(net, env, brain_name, agent, count=10, device="cpu"):
    rewards = 0.0
    steps = 0
...
def runnerD4PG(args):
    ...
    act_net = model.DDPGActor(model_args).to(device)
    crt_net = model.D4PGCritic(model_args).to(device)
    print(act_net)
    print(crt_net)
    tgt_act_net = model.TargetNet(act_net)
    tgt_crt_net = model.TargetNet(crt_net)

    writer = SummaryWriter(comment="-d4pg_train")
    agent = agents.AgentDDPG(act_net, device=device)
    buffer = experience.PrioritizedReplayBuffer(buffer_size=buffer_size, alpha = alpha)
        ...
        with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            ...
            while True:
                ...
                for s,a,r,ns,d in zip(states, actions, rewards, next_states, dones):
                    buffer._add(sample=Experience(s,a,r,d,ns))
                states = next_states
                ...
                if frame_idx % TEST_ITERS == 0:
                    ts = time.time()
                    rewards, steps = test_net(act_net, env, brain_name, agent, device=device)
